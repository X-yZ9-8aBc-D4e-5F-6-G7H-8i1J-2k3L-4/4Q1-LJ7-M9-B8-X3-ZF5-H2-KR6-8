...
LoadConfig() {
    global Smooth, CfovX, CfovY, ColVn, ScreenWidth, ScreenHeight, AutoPing, configFilePath
    ...
    IniRead, AutoPing, %configFilePath%, Settings, AutoPing, false  ; Lê o AutoPing
}
...
CreateConfigFile() {
    ...
    if !FileExist(configFilePath) {
        FileAppend, [Settings]`nSmooth=0.0`nCfovX=0`nCfovY=0`nColVn=0`nScreenWidth=%A_ScreenWidth%`nScreenHeight=%A_ScreenHeight%`nAutoPing=false`n, %configFilePath%
    }
}
...
Loop
{
    ...
    ; Ativa o auto ping ao pressionar o botão esquerdo do mouse se AutoPing estiver ligado
    if (AutoPing = "true" && GetKeyState("LButton", "P")) {
        if !scrollClicked {
